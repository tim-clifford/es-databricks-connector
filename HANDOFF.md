...
- **Datatype coverage is complete, with two fidelity caveats.** Every Spark column is exportable
  (verified end-to-end across all types): `coerce_value` handles all Arrow-crossable types
  (numerics, `decimal`→float, `binary`→base64, `date`/`timestamp`→epoch-millis, array/map/struct,
  plus a str fallback for anything unforeseen); `cast_unsupported_to_string(df)` handles the types
  that can't cross Arrow at all (`interval`, and use it for `variant` too). Field pruning

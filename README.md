...
Every transform above is exercised end-to-end (Spark → Arrow → bulk → ES → read back) by the
`datatype_coverage` demo in the [demos repo](https://github.com/tim-clifford/es-databricks-connector-demos),
which writes one column per Spark type — plus edge cases (non-finite floats, empty/null containers,
nested VARIANT, unicode) — and reads each back to confirm the stored value. When adding fields, add a

...
# MAGIC (`read_coerce`) is the true inverse of the write path (build_ndjson / to_json) against real
# MAGIC Elasticsearch. With the pure-Python write transform gone, this live round-trip -- not an
# MAGIC offline oracle -- is where write↔read fidelity is proven end to end.
# MAGIC
...
# The declared schema the reader must be given (no inference). Covers the invertible types and the
# documented-lossy ones. VARIANT/INTERVAL are excluded here: they read back as strings and are
# covered by test_datatype_coverage / test_sanitize_for_arrow; this fixture is about the typed
# write->read inverse.
SCHEMA = StructType([
    ...
    # timestamp_ntz: reads back NAIVE (no tzinfo), exercising read_coerce's timestamp_ntz branch
    # end-to-end against the epoch-millis build_ndjson stores.
...
class TestReadRoundtrip(NotebookTestFixture):
    ...
    def run_setup(self):
        ...
        # Write, then read back with the declared SCHEMA. Type mapping: doc_id keyword, dates as
        # epoch_millis, exact-decimal-as-string keyword.
        ...
        self.write_result = bulk_write(self.src, write_cfg)
        requests.post(f"{ES_HOSTS}/{INDEX}/_refresh", auth=ES_AUTH, verify=False, timeout=30)
        out = read_index(spark, read_cfg, SCHEMA)
        self.out_schema = out.schema
        self.out_rows = {r["doc_id"]: r.asDict(recursive=True) for r in out.collect()}
    ...
    def test_read_schema_matches_declared(self):
        ...
        assert self.out_schema == SCHEMA, self.out_schema
    ...
    def test_timestamp_ntz_roundtrip_naive(self):
        # timestamp_ntz reads back NAIVE (no tzinfo) and equal to the source wall-clock.
        for did in ("r1", "r2"):
            s, o = self.src_rows[did], self.out_rows[did]
